import numpy as np
from metaforce.pearl.rand_param_envs.base import RandomEnv
from metaforce.pearl.rand_param_envs.gym import utils
import os
...
class PR2Env(RandomEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        return np.concatenate([
            ...
            self.sim.data.qvel.flat[:7],  # Do not include the velocity of the target (should be 0).
            self.get_tip_position().flat,
            self.get_vec_tip_to_goal().flat,
        ])
    ...
    def goal(self):
        ...
        return self.sim.data.qpos.flat[-3:]
